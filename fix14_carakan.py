...
def kirim_data_img(img, waktu, tanggal,jadwal, ip):
    ...
    print(waktu, tanggal)
    data_tmp = data
    data_fix = {"foto_cam":img,"imei":imei, "waktu":waktu, "tanggal":tanggal, "jadwal:":jadwal, "ip_cctv": ip }
...
def kirim_data_sensor(data,imei, waktu, tanggal):
    ...
    print(waktu, tanggal)
    data_tmp = data
    data_fix = {"ketinggian_air":data,"imei":imei, "waktu":waktu, "tanggal":tanggal }
    crt_time = dt.datetime.now()
    #print("tes" ,data_fix)
    try:
        r = requests.post(url, data=json.dumps(data_fix), headers=headers)
